...
export default function ExamsCreatePage() {
  ...
  const handleAIPropose = async () => {
    ...
    try {
      ...
      const questions = await generateQuestions(dto);
      setAiQuestions(questions);
      pushToast('Propuesta IA generada.', 'success');
    } catch (e: any) {
      setAiQuestions([]);
      setAiError(e?.message || 'No se pudieron generar preguntas.');
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const onRegenerateAll = async () => {
    ...
    const dto = buildAiInputFromForm(data);
    const qs = await generateQuestions(dto as any);
    setAiQuestions(qs);
  };
  ...
  const onRegenerateOne = async (q: GeneratedQuestion) => {
    ...
    const [only] = await generateQuestions(oneDto as any);
    if (only) setAiQuestions(prev => prev.map(x => (x.id === q.id ? { ...only, id: q.id, include: q.include } : x)));
  };
